  ...
  fetchRoundWiseBiResult(state: any) {
    this.liveResultService.roundwiseBiResult(state ,this.formData.get('ac_no')?.value).subscribe(
      (data: any) => {
        ...
        this.roundwiseBiResult = data.map((i: any) => {
          const total_votes = this.roundwiseTotalVoter?.find(
            (j: any) => j?.AC_NO == i?.AC_NO && j?.ROUND_NO == i?.ROUND,
          )?.TOTAL_VOTERS;

          const turnout_percentage = this.turnout.find(
            (k: any) => k?.AC_NO == i?.AC_NO,
          )?.AC_TURNOUT;
          ...
          const secondMaxVotesBi = Math.max(
            ...parties
              .filter(
                (party: string) => result[party] !== maxVotesBi && result[party] !== undefined,
              )
          ...
          const totalVoters = this.liveResultData
            .filter(
              (liveresult: any) => liveresult?.ac_no == i?.AC_NO && liveresult?.ROUND == i?.ROUND,
            )
          ...
          const maxVotesActual = Math.max(
            ...parties.map((party: string) => result['Acc_' + party]),
          );
          ...
          result['TOTAL_Acc_Votes'] = totalVoters;
