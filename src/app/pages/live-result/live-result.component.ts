...
export  class LiveResultComponent implements OnInit {
  ...
  states : any;
  electionType : any = [
    { label: 'Assembly Election', value: 'assembly_election' },
  ];
    ...
    if (this.resultData) {
      ...
      let partyWiseData = uniqueParty?.map((party: any) => {
        let correct = this.resultData
          ?.filter(
            (row: any) =>
              row[pData]?.split(' ')[0] == row[compData]?.split(' ')[0]
          )
  ...
  getPartyColor_res(party: any) {
    ...
    let color =
      this.partyColor?.find((i: any) => i?.PARTY?.toLowerCase() == winnerParty)
        ?.CODES || '#ccc';
  ...
  getConclusionData() {
    ...
    let bjpActualVoteShare = `${(
      ((this.partyVote?.find(
        (party: any) => party.party === 'Bharatiya Janata Party'
      )?.VOTES || 0) /
        totalVotes) *
      100
    )?.toFixed(2)} %`;
    let aapActualVoteShare = `${(
      ((this.partyVote?.find((party: any) => party.party === 'Aam Aadmi Party')
        ?.VOTES || 0) /
        totalVotes) *
      100
    )?.toFixed(2)} %`;
    let incActualVoteShare = `${(
      ((this.partyVote?.find(
        (party: any) => party.party === 'Indian National Congress'
      )?.VOTES || 0) /
        totalVotes) *
      100
    )?.toFixed(2)} %`;
    ...
    let othersPredictedVoteShare = (100 - (Number(bjpActualVoteShare?.split(' ')[0]) + Number(aapActualVoteShare?.split(' ')[0]) + Number(incActualVoteShare?.split(' ')[0]) ))?.toFixed(2);


    this.dataSource_2 = [
      { x: 'BJP', y: Number(bjpPrecVoteshare?.split(' ')[0]) , fill: this.getPartyWiseColorCodes('BJP'), text: 'BJP' },
      { x: 'AAP', y: Number(aapPrecVoteshare?.split(' ')[0]) , fill: this.getPartyWiseColorCodes('AAP'), text: 'AAP'},
      { x: 'INC', y: Number(incPrecVoteshare?.split(' ')[0]) ,  fill: this.getPartyWiseColorCodes('INC'), text: 'INC' },
      { x: 'OTHERS', y: Number(othersPredictedVoteShare) ,  fill: this.getPartyWiseColorCodes('OTHERS'), text: 'OTHERS' },
    ];

    this.dataSource = [
      { x: 'BJP', y: Number(bjpActualVoteShare?.split(' ')[0]) , fill: this.getPartyWiseColorCodes('BJP'), text: 'BJP' },
      { x: 'AAP', y: Number(aapActualVoteShare?.split(' ')[0]) , fill: this.getPartyWiseColorCodes('AAP'), text: 'AAP'},
      { x: 'INC', y: Number(incActualVoteShare?.split(' ')[0]) ,  fill: this.getPartyWiseColorCodes('INC'), text: 'INC' },
      { x: 'OTHERS', y: Number(othersVoteShare) ,  fill: this.getPartyWiseColorCodes('OTHERS'), text: 'OTHERS' },
    ];
  ...
  removeLabel() {
    if (this.isBrowser()) {
      ...
      const banners = document.querySelectorAll('div[style*="position: fixed"][style*="top: 10px"][style*="left: 10px"][style*="right: 10px"]');
      
      banners.forEach((banner) => {
        // Check if the banner contains the specific text content
        if (banner.textContent && banner.textContent.includes('This application was built using a trial version of Syncfusion')) {
      ...
      const overlay = document.querySelector('div[style*="position: fixed"][style*="top: 0"][style*="left: 0"][style*="right: 0"][style*="bottom: 0"][style*="background-color: rgba(0, 0, 0, 0.5)"]');
